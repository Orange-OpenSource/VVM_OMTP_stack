...
public class CharsetUtil {
    ...
    static {
        ...
        for (int i = 0; i < JAVA_CHARSETS.length; i++) {
            try {
                ...
                decodingSupported.add(JAVA_CHARSETS[i].canonical.toLowerCase(Locale.getDefault()));
            } catch (UnsupportedOperationException e) {
            } catch (UnsupportedEncodingException e) {
            }
            try {
                "dummy".getBytes(JAVA_CHARSETS[i].canonical);
                encodingSupported.add(JAVA_CHARSETS[i].canonical.toLowerCase(Locale.getDefault()));
            } catch (UnsupportedOperationException e) {
            } catch (UnsupportedEncodingException e) {
            }
        }
        ...
        for (int i = 0; i < JAVA_CHARSETS.length; i++) {
            ...
            charsetMap.put(c.canonical.toLowerCase(Locale.getDefault()), c);
            if (c.mime != null) {
                charsetMap.put(c.mime.toLowerCase(Locale.getDefault()), c);
            }
            if (c.aliases != null) {
                for (int j = 0; j < c.aliases.length; j++) {
                    charsetMap.put(c.aliases[j].toLowerCase(Locale.getDefault()), c);
                }
            }
        }
    ...
    public static boolean isEncodingSupported(String charsetName) {
        return encodingSupported.contains(charsetName.toLowerCase(Locale.getDefault()));
    }
    ...
    public static boolean isDecodingSupported(String charsetName) {
        return decodingSupported.contains(charsetName.toLowerCase(Locale.getDefault()));
    }
    ...
    public static String toMimeCharset(String charsetName) {
        Charset c = (Charset) charsetMap.get(charsetName.toLowerCase(Locale.getDefault()));
        if (c != null) {
            return c.mime;
        }
        return null;
