...
public final class VoicemailImpl implements Voicemail {
     ...
     * <p>
     * The timestamp and number are mandatory.
     */
    ...
    /**
     * Create a {@link Builder} with its fields set to the values contained by the provided  
     * {@link Voicemail}.
     */
	public static Builder createCopyBuilder(Voicemail message) {
		Builder builder = new Builder().setHasContent(message.hasContent());
		
		if (message.hasDuration()) {
			builder.setDuration(message.getDuration());
		}
		if (message.hasId()) {
			builder.setId(message.getId());
		}
		if (message.hasNumber()) {
			builder.setNumber(message.getNumber());
		}
		if (message.hasRead()) {
			builder.setIsRead(message.isRead());
		}
		if (message.hasSourceData()) {
			builder.setSourceData(message.getSourceData());
		}
		if (message.hasSourcePackage()) {
			builder.setSourcePackage(message.getSourcePackage());
		}
		if (message.hasTimestampMillis()) {
			builder.setTimestamp(message.getTimestampMillis());
		}
		if (message.hasUri()) {
			builder.setUri(message.getUri());
		}
